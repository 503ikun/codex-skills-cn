...
async def get_articles(
    ...
    **查询参数：**
    ...
    """
    try:
        safe_log(f"获取文章列表: fakeid={fakeid[:8]}...")
        ...
        if base_resp.get("ret") != 0:
            ...
            safe_log(f"微信API返回错误: ret={ret_code}, msg={error_msg}")
            
            # 检查是否需要重新登录
            if "login" in error_msg.lower() or ret_code == 200003:
                auth_manager.mark_invalid("articles", f"ret={ret_code}, msg={error_msg}")
        ...
        safe_log(f"HTTP错误: {e.response.status_code}")
        return ArticlesResponse(
            success=False,
