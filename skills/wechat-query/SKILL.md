...
```powershell
& "$env:USERPROFILE\.codex\skills\wechat-query\scripts\run_obsidian_chatgpt_archive.ps1" -DryRun
```

完整链路：
1. 执行统一预检：`GET /api/health` 与 `GET /api/admin/status`，必须 `authenticated=true` 且 `loginState=valid`。
2. 对每个公众号调用 `GET /api/public/searchbiz?query=...`，取首条候选作为目标公众号，并记录 `query/nickname/alias/fakeid`。
3. 对每个 `fakeid` 调用 `GET /api/public/articles?fakeid=...&count=10`，筛选最近 24 小时文章。
4. 对命中文章调用 `POST /api/article/fetch` 获取全文，生成 `raw-metadata.json` 与 `articles-enriched.json`。
5. 生成 master prompt 与逐篇 prompt，交给 `continuous-dialogue` 的 Chrome ChatGPT helper 在同一个长对话中逐篇发送。
